...
def obtener_tema_del_texto(archivo_test, sw):
    ...
    for archivo in archivos_entrenamiento_limpios:
        hilo_preparar_archivo_para_lda = threading.Thread(target=preparar_archivo_entrenamiento_para_lda, args=(archivo.texto, nlp, sw,))
    ...
    topico_con_mas_score.extend(
        [palabra.split("*")[1].replace("\"", "") for palabra in modelo_lda.print_topic(indice, 8).split(" + ")])
